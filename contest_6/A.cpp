...
class MaxTree {
 public:
  ...
  void BuildTree(std::vector<int64_t>& numbers, Node node) {
    if (node.tl == node.tr) {
    ...
    BuildTree(numbers, left_son);
    Node right_son = {2 * node.vertex + 2, tm + 1, node.tr};
    BuildTree(numbers, right_son);
  ...
  std::pair<int64_t, int64_t> GetAns(Node node, int left, int right) {
    ...
    Node right_son = {2 * node.vertex + 2, tm + 1, node.tr};
    if (right <= tm) {
      return GetAns(left_son, left, right);
    }
    if (left >= tm + 1) {
      return GetAns(right_son, left, right);
    }
    std::pair<int64_t, int64_t> left_son_ans = GetAns(left_son, left, tm);
    std::pair<int64_t, int64_t> right_son_ans =
        GetAns(right_son, tm + 1, right);
...
int main() {
  ...
  tree.BuildTree(dp, root);
  std::sort(numbers.begin(), numbers.end(), Cmp);
  for (int i = 0; i < amount_of_numbers; ++i) {
    std::pair<int64_t, int64_t> ans = tree.GetAns(root, 0, numbers[i].second);
