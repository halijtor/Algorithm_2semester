...
std::vector<int> MakeMask(int curr_mask, int size_n) {
  int ans = 0;
  int index = -1;
...
bool IsValid(int mask, int size_n,
...
bool IsCompatible(int mask1, int mask2, int size_n) {
...
void DoingDynamic(int size_n, int size_m, int& result,
                  std::vector<std::vector<char>>& city_map,
                  std::vector<std::vector<int>>& dp_table) {
  int max_mask = 1 << size_n;
  for (int mask = 0; mask < max_mask; ++mask) {
    if (IsValid(mask, size_n, city_map, 0)) {
      dp_table[0][mask] = 1;
    }
  }
  for (int j = 1; j < size_m; ++j) {
    for (int curr_mask = 0; curr_mask < max_mask; ++curr_mask) {
      if (!IsValid(curr_mask, size_n, city_map, j)) {
        continue;
      }
      std::vector<int> prev = MakeMask(curr_mask, size_n);
      for (size_t i = 0; i < prev.size(); ++i) {
        if (IsCompatible(curr_mask, prev[i], size_n)) {
...
int main() {
  ...
  int result = 0;
  DoingDynamic(size_n, size_m, result, city_map, dp_table);
